...
std::vector<Node> inter_operator::Relocate::operator()(const Problem &problem, SpecificSolution &solution,
  ...
  for (Node route_x = 0; route_x < context.NumRoutes(); ++route_x) {
    for (Node route_y = 0; route_y < context.NumRoutes(); ++route_y) {
      ...
      if (!cache.TryReuse()) {
        RelocateInner(problem, solution, context, route_x, route_y, cache, star_caches, rand());
      } else {
