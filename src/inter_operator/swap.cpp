...
#include "../../include/base_cache.h"
  ...
  template <int num_x, int num_y>
  void UpdateShift(const Instance &instance, const SpecificSolution &solution, Node route_x, Node route_y,
  ...
  template <int num_x, int num_y>
  void UpdateSwap(const Instance &instance, const SpecificSolution &solution, Node route_x, Node route_y,
  ...
  template <int num_x, int num_y>
  void SwapInner(const Instance &instance, SpecificSolution &solution, RouteContext &context, Node route_x,
  ...
  template <int num_x, int num_y> std::vector<Node> Swap<num_x, num_y>::operator()(
      const Instance &instance, SpecificSolution &solution, RouteContext &context,
    ...
    for (Node route_x = 0; route_x < context.NumRoutes(); ++route_x) {
      for (Node route_y = num_x != num_y ? 0 : route_x + 1; route_y < context.NumRoutes();
        ...
        if (!cache.TryReuse()) {
          SwapInner<num_x, num_y>(instance, solution, context, route_x, route_y, cache);
        } else {
