    ...
    do {
      ...
      for (bool reversed : {false, true}) {
        if (predecessor_load_x + successor_load_y <= problem.capacity
          ...
          if (cache.delta.Update(delta, rand())) {
...
std::vector<Node> inter_operator::Cross::operator()(const Problem &problem, SpecificSolution &solution,
                                                    RouteContext &context, CacheMap &cache_map) const {
  ...
  for (Node route_x = 0; route_x < context.NumRoutes(); ++route_x) {
    for (Node route_y = route_x + 1; route_y < context.NumRoutes(); ++route_y) {
      ...
      if (!cache.TryReuse()) {
        CrossInner(problem, solution, context, route_x, route_y, cache, rand());
      } else {
        cache.move.route_x = route_x;
        cache.move.route_y = route_y;
      }
      if (best_delta.Update(cache.delta, rand())) {
